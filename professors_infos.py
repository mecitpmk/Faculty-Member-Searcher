...
class GraphicalInferface(Frame):
    ...
    def collect_informations(self):
        ...
        if not self.active_database:
            ...
            for user in self.all_user_dictionary:
                user_link=self.all_user_dictionary[user]['Link']
                ...
                self.user_and_keywords[user]=new_list
    ...
    def eveluate_without_department(self):
        ...
        criterias_user=[]
        for user in self.all_user_dictionary:
            department=self.all_user_dictionary[user]['Department']
    ...
    def find_persons(self):
        ...
        if not hasattr(self,"all_user_dictionary"):
            ...
            for user in criterias_user:
                user_keywords_list=self.user_and_keywords[user]
                for get_words in user_keywords_list:
    ...
    def add_to_treeView(self):
        ...
        for member,score in self.new_score.items():
            member_link=self.all_user_dictionary[member]['Link']
            self.treeView.insert("",counter,text=counter,values=(member,str(score),member_link))
            counter+=1
    ...
    def UploaddatabaseSystem(self):
        ...
        database['ProfessorLinks']=pickle.dumps(self.all_user_dictionary)
        database['ProfessorInfo']=pickle.dumps(self.user_and_keywords)
    ...
    def DownloadDatabase(self):
        ...
        """
        database=dbm.open("sehir_professors_infos.db","c")
        self.all_user_dictionary=pickle.loads(database['ProfessorLinks'])
        self.user_and_keywords=pickle.loads(database['ProfessorInfo'])
    ...
    def check_update_Requirements(self):
        ...
        for user in self.all_user_dictionary:
                ...
                spl=informations[0].text.split()
                new_list=[a.strip(",.") for a in spl]
                check_changes_dict[user]=new_list
        if check_changes_dict == self.user_and_keywords:
            return False
        else:
            return True
